...
export class brainComponent implements OnInit {
	...
	private brainTips = data.brain.tips;
	public brainTipsActive : boolean = true;
	public brainTipMessage : string = "";
	private model = new Model(data.brain.modelList,data.brain.messages);
	public bottom: string = "-400px";

	private r(e: any, t: number, o: any){
		let brainText = sessionStorage.getItem("brain-text");
		if (!e || (brainText && brainText > o)) return;

		if (this.i) {
			clearTimeout(this.i);
			this.i = null;
		}
		sessionStorage.setItem("brain-text", o);
		this.brainTipsActive = true;
		this.brainTipMessage = this.getRandomValue(e);
		this.changeDetectorRef.detectChanges();
		this.i = setTimeout(() => {
			sessionStorage.removeItem("brain-text");
			this.brainTipsActive = false;
			this.changeDetectorRef.detectChanges();
		}, t);
	}
	...
	private brainTools : any = {
		...
		'fa-info-circle': (event: any) => {
			event.preventDefault();
			open("https://github.com/nohr-neuralreef/nohr-neuralreef.github.io/");
		},
		'fa-times': (event: any) => {
			event.preventDefault();
	...
	public brainToolsKeys = ['fa-comment','fa-user-circle','fa-street-view','fa-camera-retro','fa-info-circle','fa-times'];
	...
	private loadWidget() : void{
		localStorage.removeItem("brain-display");
		sessionStorage.removeItem("brain-text");
		...
		for (let tool in this.brainTools){
			document.querySelector(`#brain-tool .${tool}`)!!.addEventListener("click",this.brainTools[tool]);
		}
		let e;
		...
		let listener = (t : any) => {
				for (let { selector: o, text: a } of this.brainTips.mouseover) {
		...
		for (let { date: date, text: t } of this.brainTips.seasons){
			const o = new Date();
